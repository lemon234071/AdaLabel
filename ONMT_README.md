This fork of OpenNMT-py is modified to suppoprt training of this [ACL 2021 paper](https://arxiv.org/abs/), which can be used through this [repo](). The differences can be viewed by comparing the [commits](). Note that this paper is developed from a relatively old version of OpenNMT-py, which was already the latest one at the time we started the project. Therefore, some new advanced features might not be supported.
